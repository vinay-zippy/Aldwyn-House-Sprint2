...
import { clearSession, getRole, getUsername } from '../../services/auth'
...
export const Header: React.FC<HeaderProps> = ({ onOpenMobileMenu }) => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    void searchGuests(searchQuery.trim()).then((guests) => {
      if (guests[0]) {
        navigate(`/guests/${encodeURIComponent(guests[0].id)}`)
        setSearchQuery('')
        setSearchError(null)
      } else setSearchError('No guest found')
  ...
  return (
    <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 md:px-8 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-3">
        ...
        {searchError && <span className="text-xs text-rose-600">{searchError}</span>}

        {getRole() === 'FRONT_DESK' && <button
          type="button"
        ...
        <div className="relative">
          ...
          {notificationsOpen && <div className="absolute right-0 top-10 z-40 w-72 rounded-xl border border-slate-200 bg-white p-4 text-xs shadow-lg"><p className="font-semibold text-slate-900">Notifications</p><p className="mt-2 text-slate-500">Room status updates will appear here.</p></div>}
        </div>
        <div className="relative"><button type="button" onClick={() => setProfileOpen((open) => !open)} className="flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700"><span className="font-semibold">{getUsername()}</span><ChevronDown className="h-3.5 w-3.5" /></button>{profileOpen && <button type="button" aria-label="Sign out" onClick={() => { clearSession(); navigate('/login', { replace: true }) }} className="absolute right-0 top-9 z-30 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold shadow-lg">Sign out</button>}</div>
      </div>
    </header>
  )
}
